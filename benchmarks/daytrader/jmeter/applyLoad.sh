...
/usr/bin/time -v \
jmeter -n -t "daytrader7.jmx" "${extra_args[@]}" -JHOST="${liberty_addr}" \
       -JPORT="${JPORT}" -JSTOCKS="${JSTOCKS}" -JBOTUID="${JBOTUID}" \
       -JTOPUID="${JTOPUID}" -JTHREADS="${JTHREADS}" -JDURATION="${JDURATION}"
